...
def validate_img(url):
    """Ensures we always return a valid string URL for st.image"""
    # 1. Check if it's actually a string
    if not isinstance(url, str):
        return FALLBACK_IMG
    
    # 2. Check for empty, 'None' strings, or broken TMDB paths
    if not url or url.strip() == "" or "None" in url or url == TMDB_IMG_BASE:
        return FALLBACK_IMG
        
    return url

def render_grid(movies, cols=5, key_p="grid"):
    # Safety: If the API failed and returned None instead of a list
    ...
    for i in range(0, len(movies), cols):
        ...
        # Extract the current batch of movies for this row
        batch = movies[i : i + cols]
        
        for j, movie in enumerate(batch):
            with cols_obj[j]:
                # Force the tmdb_id to be a string/int for the key
                m_id = movie.get("tmdb_id", f"unknown_{i}_{j}")
                
                # Get and strictly validate the poster
                raw_poster = movie.get("poster_url")
                poster = validate_img(raw_poster)
                
                st.markdown('<div class="movie-card">', unsafe_allow_html=True)
                
                # The actual line that was crashing: now protected by validate_img
                st.image(poster, use_container_width=True)
                
                title = movie.get("title", "Untitled")
                st.markdown(f'<div class="movie-title">{title}</div>', unsafe_allow_html=True)
                
                # Unique key for every button to prevent Streamlit duplicate errors
