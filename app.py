...
# CONFIG
# =============================
# Uses environment variable on Streamlit Cloud; falls back to localhost for your PC
API_BASE = os.getenv("MOVIE_API_URL", "https://movie-backend-wqs0.onrender.com").rstrip("/")
...
def validate_img(url):
    """CRITICAL: Fixes TypeError by ensuring st.image always gets a valid string"""
    if not url or url == "None" or url == TMDB_IMG_BASE or str(url).strip() == "":
        return FALLBACK_IMG
    return url
    ...
    for i in range(0, len(movies), cols):
        ...
        for j, movie in enumerate(movies[i : i + cols]):
            with cols_obj[j]:
                ...
                # Dynamic key for Streamlit buttons
                btn_key = f"btn_{key_p}_{movie.get('tmdb_id')}_{i+j}"
                if st.button("Details", key=btn_key, use_container_width=True):
                    nav_to("details", movie.get('tmdb_id'))
    ...
    if movie:
        ...
        with st.spinner("Calculating similarity..."):
            ...
            if bundle:
                ...
                with t1:
                    tfidf_cards = [r['tmdb'] for r in bundle.get("tfidf_recommendations", []) if r.get('tmdb')]
