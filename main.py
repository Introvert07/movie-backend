...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if r.status_code != 200:
        return {"results": [], "id": 0} # Safe fallback
...
async def tmdb_search_first(query: str) -> Optional[dict]:
    data = await tmdb_get("/search/movie", {"query": query, "include_adult": "false", "language": "en-US", "page": 1})
...
def get_local_idx_by_title(title: str) -> Optional[int]:
    ...
    if df is not None:
        matches = df[df['title'].str.lower().str.contains(norm_q, na=False)]
...
async def search_bundle(query: str = Query(..., min_length=1), tfidf_top_n: int = Query(12, ge=1, le=30), genre_limit: int = Query(12, ge=1, le=30)):
    ...
    # 1) TF-IDF recommendations (Content-based)
    tfidf_items: List[TFIDFRecItem] = []
    recs = tfidf_recommend_titles(details.title, top_n=tfidf_top_n)
    
    # Fallback to query if exact title fails
    if not recs:
    ...
    # 2) Genre recommendations (TMDB discover)
